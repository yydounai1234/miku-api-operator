...
def test_get_stream_info(bucket_name, stream_key):
    """
    测试获取流信息接口
    ...
    # info 参数必填，按文档放在查询串
    url = "http://{}{}?info=".format(host, path)
...
if __name__ == "__main__":
    # 测试获取流信息，替换为实际空间名称和流名称
    bucket_name = "test-bucket-name"
    stream_key = "test-stream"
    print("Testing get stream info API with bucket name: {}, stream key: {}".format(bucket_name, stream_key))
    response = test_get_stream_info(bucket_name, stream_key)
