...
def test_create_bucket(bucket_name):
    ...
    Args:
        bucket_name (str): 要创建的空间名称
    """
    
    # 接口信息
    method = "PUT"
    url = "http://{}.mls.cn-east-1.qiniumiku.com/".format(bucket_name)
...
if __name__ == "__main__":
    # 测试创建空间，替换 "test-bucket-name" 为你想要创建的空间名称
    bucket_name = "test-bucket-name"
    print("Testing create bucket API with bucket name: {}".format(bucket_name))
    response = test_create_bucket(bucket_name)
