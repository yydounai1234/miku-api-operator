...
def test_get_upstream_domain_config(bucket_name, domain):
    """
    测试获取上行域名配置接口
    ...
    # 构建查询参数，对域名进行URL编码
    path = "/?pushDomainConfig&name={}".format(quote(domain))
...
if __name__ == "__main__":
    # 测试获取上行域名配置，替换为实际的空间名称和域名
    bucket_name = "test-bucket-name"
    domain = "test-bucket-publish.qnsdk.com"
    
    print("Testing get upstream domain config API with bucket name: {}, domain: {}".format(bucket_name, domain))
    
    response = test_get_upstream_domain_config(bucket_name, domain)
