...
def test_unbind_downstream_domain(bucket_name, domain):
    """
    测试解绑下行域名接口
    ...
    # 构建查询参数，对域名进行URL编码
    path = "/?domain&name={}".format(quote(domain))
...
if __name__ == "__main__":
    # 测试解绑下行域名，替换为实际的空间名称和域名
    bucket_name = "test-bucket-name"
    domain = "test-bucket-play.qnsdk.com"
    
    print("Testing unbind downstream domain API with bucket name: {}".format(bucket_name))
    print("Domain: {}".format(domain))
    
    response = test_unbind_downstream_domain(bucket_name, domain)
