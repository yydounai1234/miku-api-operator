...
def test_get_downstream_domain_config(bucket_name, domain):
    """
    测试获取下行域名配置接口
    ...
    # 构建查询参数，对域名进行URL编码
    path = "/?domainConfig&name={}".format(quote(domain))
...
if __name__ == "__main__":
    # 测试获取下行域名配置，替换为实际的空间名称和域名
    bucket_name = "test-bucket-name"
    domain = "test-bucket-play.qnsdk.com"
    
    print("Testing get downstream domain config API with bucket name: {}, domain: {}".format(bucket_name, domain))
    
    response = test_get_downstream_domain_config(bucket_name, domain)
