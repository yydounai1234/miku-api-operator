...
def test_list_domain_certificates(bucket_name, domain):
    """
    测试列举域名证书接口
    ...
    path = "/?domainCertificate&domain={}".format(domain)
...
if __name__ == "__main__":
    # 测试列举域名证书，替换 "test-bucket-name" 为你想要使用的空间名称
    bucket_name = "test-bucket-name"
    domain = "test-bucket-play.qnsdk.com"
    print("Testing list domain certificates API with bucket name: {} and domain: {}".format(bucket_name, domain))
    
    print("\n--- 列举下行域名证书 ---")
    response = test_list_domain_certificates(bucket_name, domain)
    print("列举下行域名证书响应内容: {}".format(response))
    ...
    print("\n--- 列举上行域名证书 ---")
    response = test_list_domain_certificates(bucket_name, domain)
    print("列举上行域名证书响应内容: {}".format(response))