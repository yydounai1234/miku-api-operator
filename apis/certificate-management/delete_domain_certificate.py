...
def test_delete_domain_certificate(bucket_name, domain, cert_name):
    """
    测试删除域名证书接口
...
if __name__ == "__main__":
    # 测试删除域名证书，替换 "test-bucket-name" 为你想要使用的空间名称
    bucket_name = "test-bucket-name"
    domain = "test-bucket-play.qnsdk.com"
    cert_name = "testbucketplay.qnsdk.com"
    print("Testing delete domain certificate API with bucket name: {}, domain: {}, cert name: {}".format(bucket_name, domain, cert_name))  
    print("--- 删除下行域名证书 ---")
    response = test_delete_domain_certificate(bucket_name, domain, cert_name)
    print("删除下行域名证书响应内容: {}\n".format(response))
    
    bucket_name = "test-bucket-name"
    domain = "test-bucket-publish.qnsdk.com"
    cert_name = "testbucketpublish.qnsdk.com"
    print("Testing delete domain certificate API with bucket name: {}, domain: {}, cert name: {}".format(bucket_name, domain, cert_name)) 
    print("--- 删除上行域名证书 ---")
    response = test_delete_domain_certificate(bucket_name, domain, cert_name)
    print("删除上行域名证书响应内容: {}".format(response))